...
${requestData.additional_preferences && requestData.additional_preferences.trim() ? `- **INCORPORATING CLIENT PREFERENCES: The client has mentioned these highlights/preferences: "${requestData.additional_preferences}". These are important to the client, but they are highlights to incorporate into well-rounded itineraries - NOT the entire focus. IMPORTANT: These preferences can be mixed in different places throughout each itinerary - they do NOT need to be visited in order or all together. Create comprehensive itineraries that include these preferences along with other diverse experiences, activities, and locations. Each itinerary should be balanced and showcase the best of Sri Lanka while incorporating the client's specific interests. Plan your routes based on geographic flow (no backtracking), and incorporate preferences wherever they naturally fit along those routes. The 3 options can emphasize different aspects, but all should be well-rounded.**` : ''}
